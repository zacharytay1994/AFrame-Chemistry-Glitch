// Source: https://github.com/c-frame/aframe-extras/blob/master/src/misc/sphere-collider.js
/**
...
window.AFRAME.registerComponent('sphere-collider', {
  schema: {
  ...
  tick: (function () {
    const position = new window.THREE.Vector3(),
        meshPosition = new window.THREE.Vector3(),
        colliderScale = new window.THREE.Vector3(),
        size = new window.THREE.Vector3(),
        box = new window.THREE.Box3(),
        collisions = [],
        distanceMap = new Map();
  ...
  handleHit: function (targetEl) {
    if (targetEl != this.el) {
      // this.el.emit('hit', {hit_by: targetEl});
      targetEl.emit('hit', { hit_by: this.el });
